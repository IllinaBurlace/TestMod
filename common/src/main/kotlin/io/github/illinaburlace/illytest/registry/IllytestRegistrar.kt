package io.github.illinaburlace.illytest.registry
...
typealias RegistrarEntry<T> = IllytestRegistrar<T>.Entry<out T>

abstract class IllytestRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Illytest.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is IllytestRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
